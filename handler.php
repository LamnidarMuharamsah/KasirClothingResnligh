...
if (isset($_GET['action'])) {
	...
	if ($action=="tambah_barang") {
		...
		// 		print_r($_POST['stok']);
		// 		print_r($_POST['harga_beli']);
		// 		print_r($_POST['harga_jual']);
		// 		print_r($_POST['kategori']);
		$root->tambah_barang($_POST['id_barang'],$_POST['nama_barang'],$_POST['stok'],$_POST['harga_beli'],$_POST['harga_jual'],$_POST['kategori']);		
	}
	...
	if ($action=="selesai_transaksi") {
		...
		while ($f=$query2->fetch_assoc()) {
			$root->con->query("insert into sub_transaksi set id_barang='$f[id_barang]',id_transaksi='$id_transaksi2',jumlah_beli='$f[jumlah_beli]',total_harga='$f[total_harga]',no_invoice='$trx', bayar='$bayar' , diskon='$diskon'");
		}
