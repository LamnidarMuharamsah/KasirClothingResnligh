	...
	<div class="padding">
		<div class="bgwhite">
			<div class="padding">
				...
				$data=$root->con->query("select barang.nama_barang,barang.harga_jual,sub_transaksi.jumlah_beli,sub_transaksi.total_harga,sub_transaksi.bayar,sub_transaksi.diskon from sub_transaksi inner join barang on barang.id_barang=sub_transaksi.id_barang where sub_transaksi.id_transaksi='$_GET[id_transaksi]'");
				$getsum=$root->con->query("select sum(total_harga) as grand_total,sum(jumlah_beli) as jumlah_beli, (sum(total_harga)*diskon/100) as potongan  from sub_transaksi where id_transaksi='$_GET[id_transaksi]'");
				...
				?>
				<tr>
					<td></td><td></td><td></td><td>Total :</td><td> Rp. <?= number_format($getsum1['grand_total']) ?></td>
				</tr>
				
					<td></td><td></td><td></td><td>Diskon :</td><td><?= number_format($diskon) ?>%</td>
				</tr>
				<tr>
					<td></td><td></td><td></td><td>Grand Total :</td><td> Rp. <?= number_format($getsum1['grand_total']-$getsum1['potongan']) ?></td>
				</tr>
				<tr>
				<tr>
					<td></td><td></td><td></td><td>Bayar :</td><td> Rp. <?= number_format($bayar) ?></td>
				</tr>
				<tr>
					<td></td><td></td><td></td><td>Kembalian :</td><td> Rp. <?= number_format($bayar -($getsum1['grand_total']-$getsum1['potongan'])) ?></td>
				</tr>
